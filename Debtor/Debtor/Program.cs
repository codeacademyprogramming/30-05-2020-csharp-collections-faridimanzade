...
class Program
{
    ...
    static void Main()
    {
        ...
        Console.WriteLine("=========== Debtors with the mail rhyta.com/dayrep.com are: ");
        List<Debtor> byEmailResult = debtors.FindAll(x => x.Email.Contains("rhyta.com") || x.Email.Contains("dayrep.com"));
        Printer(byEmailResult);



        //3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
        Console.WriteLine("=========== Debtors with the age between 26-36 are: ");
        DateTime timeNow = DateTime.Now;
        List<Debtor> byAgeResult = debtors.FindAll(x => (timeNow - x.BirthDay).TotalDays / 365 < 36 || (timeNow - x.BirthDay).TotalDays / 365 < 36);
        Printer(byAgeResult);



        //4) Borcu 5000 - den cox olmayan borclularic cixartmag
        Console.WriteLine("=========== Debtors with the debt less than 5000 are: ");
        List<Debtor> byDebtAmount = debtors.FindAll(x => x.Debt < 5000);
        Printer(byDebtAmount);



        //5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
        Console.WriteLine("=========== Debtors with the name characters less than 18 and phone number containes 7 more than 2 times: ");
        List<Debtor> byNameCharacter = debtors.FindAll(x => x.FullName.Length > 18);
        List<Debtor> copyResult = new List<Debtor>();

        for (int i = 0; i < byNameCharacter.Count; i++)
        {
            int counter = 0;
            for (int j = 0; j < byNameCharacter[i].Phone.Length; j++)
            {
                if (byNameCharacter[i].Phone[j] == '7')
                {
                    counter++;
                }
            }
            if (counter >= 2)
            {
                copyResult.Add(byNameCharacter[i]);
            }
        }
        Printer(copyResult);
        ...
        Console.WriteLine("\n\n=========== Debtors born in Winter are: ");
        List<Debtor> winterBorns = debtors.FindAll(x => x.BirthDay.Month == 1 || x.BirthDay.Month == 2 || x.BirthDay.Month == 12);
        Printer(winterBorns);
        ...
        Console.WriteLine("\n\n=========== Debtors whose debt is greater than average: ");
        var sum = 0;
        foreach (var item in debtors)
        {
            sum += item.Debt;
        }
        var avrgDebt = sum / debtors.Count;
        List<Debtor> greaterThanAvrg = debtors.FindAll(x => x.Debt > avrgDebt);
        Printer(greaterThanAvrg);

        var sortedBySurname = greaterThanAvrg.Select(x => x.FullName).ToList();
        sortedBySurname.Sort();
        foreach (var item in sortedBySurname)
        {
        ...
        var sortedByDebt = greaterThanAvrg.Select(x => x.Debt).ToList();
        sortedByDebt.Sort();
        sortedByDebt.Reverse();
        foreach (var item in sortedByDebt)
        {
